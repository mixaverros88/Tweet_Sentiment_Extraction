...
from utils.functions import tokenizing_dataframe, get_column_values_as_np_array, convert_data_frame_to_list, \
    count_word_occurrences, remove_words_from_corpus, convert_list_of_tuples_to_list, \
...
from utils.serializedModels import word2vec_logistic_regression_under_sampling, word2vec_svm_under_sampling, \
    ...
    word2vec_decision_tree_under_sampling, word2vec_k_neighbors_under_sampling
import configparser
...
# Vectorized - Word2Vec
tokenized_sentences = tokenizing_dataframe(train_data_frame_under_sampling)
...
ComposeMetrics(
    word2vec_logistic_regression_under_sampling(),
    X_train,
    y_train,
...
svm_model = SvmModel(
    ...
    y_train,
    y_test,
    config.get('MODELS', 'under_sampling.word2vec.svm'),
    svm_params)

svm_y_predict = svm_model.results()

ComposeMetrics(
    word2vec_svm_under_sampling(),
    X_train,
    y_train,
    svm_y_predict.score,
    y_test,
...
decision_tree = DecisionTreeModel(
    ...
    y_train,
    y_test,
    config.get('MODELS', 'under_sampling.word2vec.dt'),
    decision_tree_params)

decision_tree_predict = decision_tree.results()

ComposeMetrics(
    word2vec_decision_tree_under_sampling(),
    X_train,
    y_train,
...
ComposeMetrics(
    word2vec_k_neighbors_under_sampling(),
    X_train,
    y_train,
