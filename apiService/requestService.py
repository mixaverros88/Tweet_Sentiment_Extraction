from ClassificationDto import ClassificationDto
from utils.clean_dataset.DataCleaning import DataCleaning
from utils.helper_functions.functions import convert_text_to_vector_array_by_word2vec_model, \
    convert_text_to_data_frame_of_one_row
from utils.retrieve.serializedModels import bag_of_words_over_sampling, \
...
def classify_text(requested_text):
    # convert text to data frame with one row since DataCleaning accepts dataframe
    ...
    # Bag of Words
    bag_of_words_model = bag_of_words_over_sampling()
    bag_of_words_vectors = bag_of_words_model.transform([cleaned_text])

    bag_of_words_logistic_regression_model = bag_of_words_logistic_regression_over_sampling()
    ...
    bag_of_words_svm_model = bag_of_words_svm_over_sampling()
    bag_of_words_svm_results = bag_of_words_svm_model.predict(bag_of_words_vectors)

    bag_of_words_nb_model = bag_of_words_nb_over_sampling()
    bag_of_words_nb_results = bag_of_words_nb_model.predict(bag_of_words_vectors.toarray())

    bag_of_words_mlp_model = bag_of_words_multi_layer_perceptron_classifier_over_sampling()
    bag_of_words_mlp_results = bag_of_words_mlp_model.predict(bag_of_words_vectors)

    bag_of_words_decision_tree_model = bag_of_words_decision_tree_over_sampling()
    ...
    np_vec = convert_text_to_vector_array_by_word2vec_model(word2vec_model, cleaned_text)

    word2vec_logistic_regression_model = word2vec_logistic_regression_over_sampling()
    ...
    word2vec_svm_model = word2vec_svm_over_sampling()
    word2vec_svm_results = word2vec_svm_model.predict(np_vec)

    word2vec_mlp_model = word2vec_multi_layer_perceptron_classifier_over_sampling()
    word2vec_mlp_results = word2vec_mlp_model.predict(np_vec)

    word2vec_decision_tree_model = word2vec_decision_tree_over_sampling()
    ...
    tfidf_logistic_regression_model = tfidf_logistic_regression_over_sampling()
    ...
    tfidf_svm_model = tfidf_svm_over_sampling()
    tfidf_svm_results = tfidf_svm_model.predict(tfidf_model_vectors)

    tfidf_words_nb_model = tfidf_nb_over_sampling()
    tfidf_nb_results = tfidf_words_nb_model.predict(tfidf_model_vectors.toarray())

    tfidf_mlp_model = tfidf_multi_layer_perceptron_classifier_over_sampling()
    tfidf_mlp_results = tfidf_mlp_model.predict(tfidf_model_vectors)

    tfidf_decision_tree_model = tfidf_decision_tree_over_sampling()
    ...
    return classification_dto.get_response()
